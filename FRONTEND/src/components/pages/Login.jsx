    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.rol === 'customer') {
          navigate('/products'); // Redirige a la página de productos si el rol es "customer"
        } else if (data.rol === 'admin') {
          navigate('/inicioAdmin'); // Redirige a la página de InicioAdmin si el rol es "Admin"
        }
      } else {
        const errorData = await response.json();
        // Si hay errores de validación, muestra el mensaje de error en el modal
        setValidaciones(errorData.validaciones);
      }
    } catch (error) {
