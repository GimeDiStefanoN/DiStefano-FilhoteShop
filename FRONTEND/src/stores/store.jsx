...
export const cartStore = create((set) => ({
  cart: localStorage.getItem('carrito_Compras')
    ? JSON.parse(localStorage.getItem('carrito_Compras'))
    : [],
  ...
  agregarAlCarrito: (product) => {
    set((state) => ({
      cart: [...state.cart, product], // Cambiar "products" a "cart"
    }));
  },

  eliminarDelCarrito: (id) => {
    set((state) => ({
      cart: state.cart.filter((product) => product.id !== id), // Cambiar "products" a "cart"
    }));
  },

  guardarCarrito: () => {
    set((state) => {
      localStorage.setItem('carrito_Compras', JSON.stringify(state.cart)); // Cambiar "products" a "cart"
    });
  },
}));
